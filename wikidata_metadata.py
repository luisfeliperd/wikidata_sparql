...
def reconcile_database(database):
    ...
    result_df = pd.DataFrame(columns = ['metadado','instance','qid_instance','ocurrences'])
    ...
    for metadado in list(set(wikiObj_df['metadado'])):
        ...
        for index, value in meta_df['qid_instancia'].value_counts().head(5).items():
            ...
            result_df = result_df.append({'metadado':meta_df.at[meta_df_idx,'metadado'],
                                         ...
                                         'ocurrences':value}, ignore_index=True)
    return result_df

reconcile_database(tabela_df).to_csv("resultado.csv")
